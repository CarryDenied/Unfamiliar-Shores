...
namespace ACE.Server.WorldObjects
{
    partial class Player
    {
        ...
        public void HandleActionUseWithTarget(uint sourceObjectGuid, uint targetObjectGuid, bool useDoneRequired = true)
        {
            ...
            if (sourceItem == null)
            {
                log.Warn($"{Name}.HandleActionUseWithTarget({sourceObjectGuid:X8}, {targetObjectGuid:X8}): couldn't find source:{sourceObjectGuid:X8}");
            ...
            if (target == null)
            {
                log.Warn($"{Name}.HandleActionUseWithTarget({sourceObjectGuid:X8}, {targetObjectGuid:X8}): couldn't find target:{targetObjectGuid:X8}");
                SendUseDoneOrWeenieErrorEvent(useDoneRequired);
                return;
            }
