    ...
    public static class AuctionHouse
    {
        ...
        public static void PlaceBid(Player bidder, int auctionId, int bidAmount)
        {
            lock (auctionLock)
            {
                ...
                string bidderIp = bidder.Session.EndPointC2S.Address.ToString();
                    ...
                    foreach (var auction in expiredAuctions)
                    {
                        try
                        {
                            ...
                            if (auction.HighestBid > 0) // **Item was sold**
                            {
                                ...
                                if (seller != null)
                                {
                                    if (!seller.TryCreateInInventoryWithNetworking(coinItem))
                                    {
                                        StorePendingAuctionReturn(sellerGuid, coinItem, "SELLER", sellerGuid);
                                        seller.SendMessage($"[AUCTION SOLD] Your item {auction.Item.NameWithMaterial} was sold for {auction.HighestBid}, but coins were stored because your inventory was full.");
                                    }
                                    else
                                    {
        ...
        public static bool CancelBid(Player bidder, int auctionId)
        {
            lock (auctionLock)
            {
                ...
                bool validPreviousBidder = previousBidder != null && auction.PreviousBidAmount > 0;
                ...
                if (validPreviousBidder)
                {
                    ...
                    {
                        // If unable to deduct, treat as no valid previous bidder
        ...
        public static void NotifySellerAuctionExpired(AuctionItem auction)
        {
            if (auction.Seller?.DiscordUserId != null)
            {
              ...
              //  Console.WriteLine($"[AUCTION LOG] Seller Discord User ID: {auction.Seller.DiscordUserId}");
                ...
                DiscordChatBridge.SendDiscordDM(auction.Seller.Name, message, (long)auction.Seller.DiscordUserId);
            }
        }
